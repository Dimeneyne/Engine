...
extern u32 app_inactive_time_start;
void CInput::MouseUpdate()
{
	...
	{
		while (ShowCursor(TRUE) < 0);
		ShowWindow(Device.m_hWnd, SW_MINIMIZE);
		SetCursorPos(mouse_point.x, mouse_point.y);
		ShowWindow(Device.m_hWnd, SW_SHOWNOACTIVATE);
	}
	else
		while (ShowCursor(FALSE) > 0);
}
...
void CInput::OnScreenResolutionChanged()
{
	GetClientRect						(Device.m_hWnd, &screen_rect);

	POINT ul							= { screen_rect.left, screen_rect.top };
	POINT lr							= { screen_rect.right, screen_rect.bottom };

	MapWindowPoints						(Device.m_hWnd, nullptr, &ul, 1);
	MapWindowPoints						(Device.m_hWnd, nullptr, &lr, 1);

	screen_rect.left					= ul.x;
	screen_rect.top						= ul.y;
	screen_rect.right					= lr.x;
	screen_rect.bottom					= lr.y;
}
